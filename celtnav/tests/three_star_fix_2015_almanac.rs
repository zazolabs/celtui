...
//!
//! This test uses the correct star positions from the 2015 nautical almanac
//! instead of the epoch 2024 values in our catalog.
...
#[test]
fn test_three_star_fix_with_2015_almanac() {
    // Common parameters
    ...
    println!("\n=== THREE STAR FIX - 2015 ALMANAC DATA ===");
    ...
    // Calculate GHA Aries at this time
    ...
    let pollux_lha = (gha_pollux + dr_lon + 360.0) % 360.0;
    let pollux_lha_aries = (gha_aries_pollux + dr_lon + 360.0) % 360.0;
    ...
    println!("  GHA Aries: {:.4}°, LHA Aries: {:.1}° (rounds to {}°)",
             gha_aries_pollux, pollux_lha_aries, pollux_lha_aries.round() as i32);
    println!("  GHA: {:.4}°, LHA: {:.4}°", gha_pollux, pollux_lha);
    ...
    let hamal_lha = (gha_hamal + dr_lon + 360.0) % 360.0;
    let hamal_lha_aries = (gha_aries_hamal + dr_lon + 360.0) % 360.0;
    ...
    println!("\nHAMAL @ 06:32:22 UTC");
    println!("  GHA Aries: {:.4}°, LHA Aries: {:.1}° (rounds to {}°)",
             gha_aries_hamal, hamal_lha_aries, hamal_lha_aries.round() as i32);
    println!("  GHA: {:.4}°, LHA: {:.4}°", gha_hamal, hamal_lha);
    ...
    let deneb_lha = (gha_deneb + dr_lon + 360.0) % 360.0;
    let deneb_lha_aries = (gha_aries_deneb + dr_lon + 360.0) % 360.0;
    ...
    println!("\nDENEB @ 06:33:47 UTC");
    println!("  GHA Aries: {:.4}°, LHA Aries: {:.1}° (rounds to {}°)",
             gha_aries_deneb, deneb_lha_aries, deneb_lha_aries.round() as i32);
    println!("  GHA: {:.4}°, LHA: {:.4}°", gha_deneb, deneb_lha);
